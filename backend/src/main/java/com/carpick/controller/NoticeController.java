...
public NoticeController(NoticeAdminLoginService noticeAdminLoginService, NoticeAdminLoginService noticeAdminLoginService) {
...
return ResponseEntity.ok(noticeAdminLoginService.getAllNotices());
}

// 관리자 공지 상세
@GetMapping("/admin/notices/{id}")
public ResponseEntity<NoticeNtt> getNotice(@PathVariable Long id) {
return noticeAdminLoginService.getNotice(id)
...
NoticeNtt created = noticeAdminLoginService.createNotice(payload.getTitle(), payload.getContent());
...
return noticeAdminLoginService.updateNotice(id, payload.getTitle(), payload.getContent())
.map(ResponseEntity::ok)
.orElse(ResponseEntity.notFound().build());
}
...
boolean ok = noticeAdminLoginService.softDeleteNotice(id);
